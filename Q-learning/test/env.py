...
class Environment(tk.Tk, object):
    def __init__(self):
        ...
        #elf.action_space = ['up', 'down', 'left', 'right']
        self.action_space = ['left', 'right','None', 'speedup', 'slowdown']
    ...
    def build_environment(self):
        self.canvas_widget = tk.Canvas(self,  bg='Gainsboro',
        ...
        for column in range(0, env_width * pixels, pixels):
            ...
            self.canvas_widget.create_line(x0, y0, x1, y1, fill='white',width=1,dash=(10,8))
        ...
        self.obstacle1 = self.canvas_widget.create_image(pixels * 0, pixels * 2, anchor='nw', image=self.obstacle1_object)
        # self.obstacle2 = self.canvas_widget.create_image(pixels * 2, pixels * 1, anchor='nw', image=self.obstacle5_object)
        self.obstacle3 = self.canvas_widget.create_image(pixels * 0, pixels * 6, anchor='nw', image=self.obstacle3_object)
        self.obstacle4 = self.canvas_widget.create_image(pixels * 1, pixels * 7, anchor='nw', image=self.obstacle1_object)
        self.obstacle5 = self.canvas_widget.create_image(pixels * 2, pixels * 2, anchor='nw', image=self.obstacle5_object)
        self.obstacle6 = self.canvas_widget.create_image(pixels * 0, pixels * 11, anchor='nw', image=self.obstacle2_object)
        self.obstacle7 = self.canvas_widget.create_image(pixels * 2, pixels * 14, anchor='nw', image=self.obstacle3_object)
        self.obstacle8 = self.canvas_widget.create_image(pixels * 0, pixels * 15, anchor='nw', image=self.obstacle4_object)
        self.obstacle9 = self.canvas_widget.create_image(pixels * 1, pixels * 17, anchor='nw', image=self.obstacle1_object)
        # self.obstacle10 = self.canvas_widget.create_image(pixels * 1, pixels * 1, anchor='nw', image=self.obstacle1_object)
        self.obstacle11 = self.canvas_widget.create_image(pixels * 1, pixels * 4, anchor='nw', image=self.obstacle2_object)
        self.obstacle12 = self.canvas_widget.create_image(pixels * 2, pixels * 10, anchor='nw', image=self.obstacle4_object)
        self.obstacle13 = self.canvas_widget.create_image(pixels * 1, pixels * 13, anchor='nw', image=self.obstacle5_object)
        # Obstacle 2
        ...
        self.flags = [self.canvas_widget.create_image(pixels * i, pixels * 19, anchor='nw', image=self.flag_object) for i in range(env_width)]
    ...
    def reset(self):
        ...
        # Return observation
        return [*self.canvas_widget.coords(self.agent), 0]

    def check_if_hit(self, last, cur):
        obs = [self.canvas_widget.coords(self.obstacle1),
    ...
    def step(self, action):
        ...
        # 检查是否撞车
        if self.check_if_hit(state, base_action):
                reward = -100
                next_state = 'obstacle'
        ...
        # if action == 'slowdown':
        #     return next_state, 0, True if next_state in [self.canvas_widget.coords(flag) for flag in self.flags] else False

        # Updating key for the dictionary
        self.i += 1

        # Calculating the reward for the agent
        if next_state in [self.canvas_widget.coords(flag) for flag in self.flags]:
            reward = 100
            done = True
            next_state = 'goal'
