...
class Stats:
    ...
    def setBrowerPath(self):
        download_path = QtWidgets.QFileDialog.getOpenFileName(None, '浏览', 'C:/Users/szh/Desktop/szh/Python_test/GA/Digital_image_fitting')
...
class Wmain:
    ...
    def onButtonClick(self):
        ...
        if n==1:
            ...
            data, size = mnist.GetImage(ori_img)
            if resume and os.path.exists(backup):
                genes, generation, best, worst, avg= mnist.ReadData(backup)
            else:
                genes = mnist.RandGenes(size,data)
                generation = 0
                best=[]
                worst=[]
                avg=[]
    ...
    def run(self):
        ...
        global backup
        generation += 1
        genes = mnist.CalcFitness(genes, data)
        fitsum=0
        for i in range (100):
            fitsum+=genes[i][1]
        ...
        genes = mnist.Select(genes, size)
        genes = mnist.Variation(genes,data,generation)
        #if generation % 1 == 0:
        mnist.SaveData({'gene': genes, 'generation': generation, 'best':best, 'worst':worst, 'avg':avg}, backup)
        mnist.SavePic(genes[0], generation, ori_img)
        #展示图片
...
if __name__ == '__main__':
    app = QApplication([])
    app.setWindowIcon(QIcon(os.path.split(os.path.realpath(__file__))[0]+'/brain.ico'))
    stats = Stats()
    stats.ui.show()
    wmain=Wmain()
